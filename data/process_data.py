...
class MoliDataProcessor:
    ...
    def __init__(self, data_path="/home/sky/Projects/Moli-PWA/data/raw"):
        self.data_path = Path(data_path)
        
        # Find the files dynamically to handle name variations
        files = list(self.data_path.glob("*.xlsx"))
        
        billing_candidates = [f for f in files if "Facturación" in f.name or "molinos" in f.name.lower()]
    ...
    def process_billing_data(self):
        ...
        df = pd.read_excel(self.billing_file, header=2)
    ...
    def process_geographic_data(self):
        ...
        df_raw = pd.read_excel(self.sales_file, header=None)
    ...
    def generate_ml_features(self, df):
        ...
        customer_stats = df.groupby('razon_social').agg({
        ...
        product_stats = df.groupby('producto_limpio').agg({
            'monto_ars': ['sum', 'mean', 'count'],
            'total_kg': ['sum', 'mean'],
            'precio_por_kg': 'mean'
        }).round(2)
    ...
    def create_recommendation_matrices(self, df):
        ...
        customer_zone_matrix = df.pivot_table(
            index='razon_social',
            columns='zona',
            values='monto_ars',
            aggfunc='sum',
...
def main():
    ...
    rec_matrices = processor.create_recommendation_matrices(billing_df)
    
    # Generate business insights
    insights = processor.generate_business_insights(billing_df)
    ...
    return billing_df, geo_df, ml_features, rec_matrices, insights
