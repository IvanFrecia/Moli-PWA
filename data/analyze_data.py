...
def analyze_excel_files():
    ...
    base_path = Path("/home/sky/Projects/Moli-PWA/data/raw")
    
    # File paths (handling exact names with spaces)
    billing_file = base_path / "Listado de Facturación de Molinos .xlsx"
    sales_file = base_path / "Datos Basicos Ventas.xlsx"
    ...
    try:
        # Read billing data
        billing_df = pd.read_excel(billing_file) # type: ignore
        ...
        print(f"📊 Data Types:")
        for col, dtype in billing_df.dtypes.items(): # type: ignore
            print(f"   • {col}: {dtype}")
        ...
        print(f"\n❌ Missing Values:")
        missing = billing_df.isnull().sum() # type: ignore
        for col, count in missing.items():  # type: ignore
    ...
    try:
        ...
        # Check for missing values
        print(f"\n❌ Missing Values:")
        missing = sales_df.isnull().sum()   # type: ignore
        for col, count in missing.items():  # type: ignore
            if count > 0:
